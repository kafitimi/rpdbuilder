...
\subsection{Цель освоения и краткое содержание дисциплины}
  ...
  \textit{Краткое содержание дисциплины.} %{ for item in АннотацияКраткоесодержание %} <= item =>
  %{ endfor %}
  %{ endif %}
...
%{ endfor %}
\hline
%{ elif ПоказателиОцениванияПодробноТабл %}
%{ for row in ПоказателиОцениванияПодробноТабл %}
%{ for cell in row %}<= cell =>%{ if not loop.last %} & %{ endif %}%{ endfor %}
\\
\hline
%{ endfor %}
